...
def train_xgboost(df, features, target):
    """Train XGBoost regressor on the given dataframe."""
    model = XGBRegressor(objective='reg:squarederror', n_estimators=100)
    model.fit(df[features], df[target])
    return model


def forecast_xgboost(model, last_known_row, features, days):
    """Generate recursive forecasts using trained XGBoost model."""
...
# -----------------------------
def predict_xgboost(df: pd.DataFrame, days_forecast: int = 7, ticker: str = ""):
    """
    Predict future stock prices using lag-based XGBoost.
    ...
    try:
        ...
        if ticker:
            model_dir = os.path.join(os.path.dirname(__file__), 'xgboost')
            os.makedirs(model_dir, exist_ok=True)
            file_name = f"{ticker}_xgboost_{last_close_date}_{target_date}.pkl"
        ...
        if model_full is None or metrics is None:
            ...
            model_val = train_xgboost(train_df, features, target)
            last_train_row = train_df.iloc[-1][features]
            val_preds = forecast_xgboost(model_val, last_train_row, features, val_size)
            metrics = calculate_metrics(test_df['close'].values, val_preds)

            # Final training on full data
            model_full = train_xgboost(df, features, target)
        ...
        final_forecast = forecast_xgboost(model_full, last_input_series, features, days_forecast)
